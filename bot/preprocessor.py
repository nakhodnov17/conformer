...
class AudioToMelSpectrogramPreprocessor(torch.nn.Module):
    ...
    def _apply_normalization(self, features, lengths, eps=1e-5):
        ...
        vrs = torch.sum(torch.pow(features - means, 2).masked_fill(mask, 0), dim=-1, keepdims=True) / (
                    lengths[:, None, None] - 1)
...
class ConvSubsampling(torch.nn.Module):
    ...
    def forward(self, x, lengths):
        ...
        x = torch.transpose(x, 1, 2).reshape(x.shape[0], x.shape[2],
                                             -1)  # Making 4D tensor 3D by combining channels and features
