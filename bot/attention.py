...
class RelPositionMultiHeadAttention(torch.nn.Module):
    ...
    def forward_qkv(self, query, key, value):
        ...
        q = torch.transpose(q.reshape(q.shape[0], q.shape[1], self.head, -1), 1,
                            2)  # reshaping to make tensor of (batch, head, time, d_k)
    ...
    def forward_attention(self, value, scores, mask):
        ...
        if mask is not None:
            ...
            ### YOUR CODE HERE
            attn = torch.nn.functional.softmax(scores, dim=-1).masked_fill(mask.unsqueeze(1),
                                                                           0)  # (batch, head, time1, time2)
        else:
